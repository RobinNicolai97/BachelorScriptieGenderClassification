...
def find_patterns(x):
	...
	for line in lines:
		...
		if ('NOUN' in gram[1] and 'PRON' in gram[1]) or ('PROPN' in gram[1] and 'PRON' in gram[1]): 
			...
			#comments above apply to all patterns	
			...
			elif 'NOUN-VERB-PRON' in string5 or 'NOUN-VERB-ADV-PRON' in string5 or 'PROPN-VERB-PRON' in string5 or 'PROPN-VERB-ADV-PRON' in string5:
				...
				if 'VERB-ADV-PRON' in string5:
					while found == False:
						if copy_pos[-3] == 'VERB' and copy_pos[-2] == 'ADV' and copy_pos[-1] == 'PRON':
							found = True
						else:
							copy_words = copy_words[:-1]
							copy_pos = copy_pos[:-1]
				elif 'NOUN-VERB-PRON' in string5:
					while found == False:
						if copy_pos[-3] == 'NOUN' and copy_pos[-2] == 'VERB' and copy_pos[-1] == 'PRON':
							found = True
						else:
							copy_words = copy_words[:-1]
							copy_pos = copy_pos[:-1]
				elif 'PROPN-VERB-PRON' in string5:
					while found == False:
						if copy_pos[-3] == 'PROPN' and copy_pos[-2] == 'VERB' and copy_pos[-1] == 'PRON':
							found = True
						else:
							copy_words = copy_words[:-1]
							copy_pos = copy_pos[:-1]
